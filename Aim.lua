...
local function calculateLeadPosition(target, ping)
    ...
    local timeToHit = distance / bulletSpeed + (ping / 1000) * 0.5
    ...
    local verticalLead = Vector3.new(0, targetVel.Y * timeToHit * 0.2, 0)
...
createSlider("LeadFactor", 0, 300, AimBotSettings.LeadFactor*100, function(v) AimBotSettings.LeadFactor = v/100 end, 1)
...
local AIM = {
    ...
    Enable = function()
        ...
        if controlElements["Enabled"] then
            controlElements["Enabled"].Text = "Enabled: true"
        end
        Notify("AIM Enabled", Color3.fromRGB(100,255,100), 2)
    end,
    ...
    Disable = function()
        ...
        enabled = false
        if controlElements["Enabled"] then
            controlElements["Enabled"].Text = "Enabled: false"
        end
        Notify("AIM Disabled", Color3.fromRGB(255,100,100), 2)
    end,
    
    Toggle = function()
        AimBotSettings.Enabled = not AimBotSettings.Enabled
        if controlElements["Enabled"] then
            controlElements["Enabled"].Text = "Enabled: "..tostring(AimBotSettings.Enabled)
        end
        Notify("AIM "..(AimBotSettings.Enabled and "Enabled" or "Disabled"), 
               AimBotSettings.Enabled and Color3.fromRGB(100,255,100) or Color3.fromRGB(255,100,100), 2)
    end,
    ...
    SetTarget = function(player)
        if player and player.Character and player.Character:FindFirstChild("Humanoid") and player.Character.Humanoid.Health > 0 then
            Plr = player
            enabled = true
            Notify("Target set: "..player.Name, Color3.fromRGB(150,200,255), 2)
            return true
        else
            Notify("Invalid target", Color3.fromRGB(255,150,150), 2)
            return false
        end
    end,
    ...
    SetFOV = function(fov)
        AimBotSettings.FOV = fov
        if controlElements["FOV"] then
            controlElements["FOV"]:FindFirstChildOfClass("TextLabel").Text = "FOV: "..fov
        end
        Notify("FOV set to: "..fov, Color3.fromRGB(200,200,100), 2)
    end,
    
    SetSmoothness = function(smooth)
        AimBotSettings.interval = smooth
        if controlElements["interval"] then
            controlElements["interval"]:FindFirstChildOfClass("TextLabel").Text = "interval: "..(smooth*1000)
        end
        Notify("Smoothness set to: "..smooth, Color3.fromRGB(200,200,100), 2)
    end,
    
    SetPrediction = function(pred)
        AimBotSettings.LeadFactor = pred
        if controlElements["LeadFactor"] then
            controlElements["LeadFactor"]:FindFirstChildOfClass("TextLabel").Text = "LeadFactor: "..(pred*100)
        end
        Notify("Prediction set to: "..pred, Color3.fromRGB(200,200,100), 2)
    end,
    
    -- Визуальные настройки
    ToggleBeam = function(state)
        if state == nil then state = not AimBotSettings.ShowBeam end
        AimBotSettings.ShowBeam = state
        if controlElements["ShowBeam"] then
            controlElements["ShowBeam"].Text = "ShowBeam: "..tostring(state)
        end
        if not state then cleanupBeam() end
        Notify("Beam "..(state and "Enabled" or "Disabled"), 
               state and Color3.fromRGB(100,255,100) or Color3.fromRGB(255,100,100), 2)
    end,
    ...
    AutoTargetClosest = function()
        local target = AIM.FindClosestTarget()
        if target then
            return AIM.SetTarget(target)
        else
            Notify("No valid targets found", Color3.fromRGB(255,150,150), 2)
            return false
        end
    end
}
...
-- Сохраняем глобальные функции
getgenv().StandaloneAim = {
    Settings = AimBotSettings,
    Notify = Notify
}

-- Добавляем AIM модуль в глобальную область
getgenv().AIM = AIM

-- Уведомление о загрузке
Notify("AIM Module Loaded! Use AIM.Enable() to start", Color3.fromRGB(100,200,255), 3)
